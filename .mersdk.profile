...
clone_src() {
	path="$ANDROID_ROOT/$3/"
	mkdir -p "$path"
	git clone --recurse -b $2 https://github.com/Linux-On-Whyred/$1 "$path" &> /dev/null
}
...
run_mic_build() {
	if [ -z $UPDATES_CHECKED ]; then
		...
		local tmp=`curl -s https://en.wikipedia.org/wiki/Sailfish_OS | grep -B 2 "^<td>Public release$" | grep "^<td>v" | tail -1` # e.g. "<td>v3.3.0.14</td>"
		tmp=${tmp#*v} # e.g. "3.3.0.14</td>"
		local LATEST_RELEASE=${tmp::${#tmp}-5} # e.g. "3.3.0.14"
		local LATEST_TOOLING=`echo $LATEST_RELEASE | cut --complement -d"." -f4-` # e.g. "3.3.0"
		local CURRENT_TOOLING=`echo $RELEASE | cut --complement -d"." -f4-` # e.g. "3.3.0"

		# Can we build latest w/ current tooling (e.g. '3.3.0' vs '3.3.0')
